...
bool HMC5883L::init() {
    ...
    if (!writeRegister(0x00, 0x70)) return false;
    if (!writeRegister(0x01, 0x20)) return false;
    if (!writeRegister(0x02, 0x00)) return false;
    return true;
}

bool HMC5883L::read(int16_t& x, int16_t& y, int16_t& z) {
    uint8_t buffer[6];
    if (!readRegisters(0x03, buffer, 6)) return false;
